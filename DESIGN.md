...
A page has one thing to do, and it is the one red key on the page, full width, near the top, in words a first-time founder understands ("Log this week", "Build it", "Fill in the numbers"). Everything else a page can do is plain red text under it or a row in a list, never a second key. What a founder does not need on the first visit folds away behind a plain "More" row. Hints are one quiet sentence on the page's own ground with an OK, never a card with a ring around it. No page opens with a picture band and a label; it opens with its title.
...
The tiled room scenes and walking crowds of the first world are retired from page headers. The map is the exception and the hero: the building as a cross-section, six rooms as floors under a rooftop sign and a strip of sky (stars, a moon and a blinking mast by night; sun and clouds by day; a water tank on the roof). Each room has its wall in the room's colour with a picture rail and a wainscot, a window onto the sky, a pendant lamp over the middle (lit and breathing only in the room you are in), a tiled floor with a rug, a low piece of furniture under the sign and a tall one across from it, a sign hung on chains with a pictogram, the name and one window per task, a brass floor plate over the doorway, and a stair down to the next floor. The founder's keeper stands in their room under a bobbing red pin; the coaches stand in the rooms they own. Under it all, the street: the front door under a striped awning with a lit pane in each leaf, a lamp with its pool of light, a planter, and people walking past. It is the one place the app is a picture, and it is drawn entirely in the pixel world.

Everything in the picture that looks like a person or a switch answers a tap: a room opens; a task window on a sign opens that task; a coach waves, says one line in a bubble and offers a chat; your own keeper jumps, waves and says where you are. Bubbles are paper-white, land in 180 ms and leave on their own.
...
Product motion: 150 to 250 ms, state only. Enter with a strong ease-out from an already-visible default; move across with ease-in-out; sheets on the iOS sheet curve. Springs settle without a bounce. A page's sections rise into place once as it arrives (`Rise`: 200 ms, 8 px, 40 ms apart) and then nothing on the page moves again on its own: a tick that is already there is simply there. What does move, and why: a key presses down onto its foot (100 ms) and comes back (140 ms); a tick lands from slightly larger when the founder makes it (a spring, 220 ms); the red seal lands on the road the same way, from above and a little askew; the opened stop on the road unfolds in 220 ms and the rows below make room (a layout transition, never a jump); the keeper breathes 1.5 px every 1.8 s; the tab bar's disc slides in 220 ms; meters draw once. The one authored moment is the opening: the lights come up, the glass tile breathes once, the keeper walks in, under 1.4 s. All of it is a cross-fade or nothing under Reduce Motion.

## Anti-references

The tells of a generated interface, each of which this world has been caught by once and must not be again: Inter as the default face; purple-to-blue or orange-to-coral gradients; glossy pills with a specular sweep; glass on every card with a glow behind it; tinted icon tiles with a tiny glyph inside; the icon-plus-heading-plus-text card as page structure; uppercase mono labels over every section; emoji or thin-line icons; a bounce on every press; an arrow → after every link and button label; middle dots · strung between every fact; three buttons where one would do; a hint card with a coloured ring at the top of every page. The retro is the answer to all of them: hard edges, a foot, a pixel pictogram, a hand-drawn keeper.
