...
function PixelChat({ size = 20, color = "var(--paper)" }: { size?: number; color?: string }) {
  const px = size / 12;
  ...
  return (
    <>
      ...
      >
        <PixelChat />
        {badge > 0 && (
          <span className="absolute -right-1.5 -top-1.5 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-accent-strong px-1 text-xs font-medium text-white">
            {badge > 9 ? "9+" : badge}
          </span>
        )}
      </button>
