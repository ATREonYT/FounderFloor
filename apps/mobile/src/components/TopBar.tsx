...
import { Keeper, Spec, Sprite, alpha, radius, scheme, shell, useLayout } from "@founderfloor/ui";
...
export function Back({ label = "Back" }: { label?: string }) {
  ...
  return (
    <Pressable hitSlop={4} onPress={() => (router.canGoBack() ? router.back() : router.replace("/today"))} accessibilityRole="button" accessibilityLabel={label} style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1, backgroundColor: alpha.wellFill(), borderWidth: 1, borderColor: alpha.hairline(), borderRadius: radius.full, paddingHorizontal: 14, height: 36, justifyContent: "center" })}>
      <Spec tone="ink" numberOfLines={1}>{`← ${label}`}</Spec>
    </Pressable>
  );
}
...
export function You() {
  ...
  if (!auth) {
    return (
      <Pressable hitSlop={4} onPress={() => router.push("/sign-in")} accessibilityRole="button" accessibilityLabel="Sign in" style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1, backgroundColor: alpha.wellFill(), borderWidth: 1, borderColor: alpha.hairline(), borderRadius: radius.full, paddingHorizontal: 14, height: 36, justifyContent: "center" })}>
...
export function Mailbox() {
  ...
  return (
    <Pressable hitSlop={4} onPress={() => router.push("/inbox")} accessibilityRole="button" accessibilityLabel={unread ? `Inbox, ${unread} unread` : "Inbox"} style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1, width: 36, height: 36, borderRadius: radius.full, backgroundColor: alpha.wellFill(), borderWidth: 1, borderColor: alpha.hairline(), alignItems: "center", justifyContent: "center" })}>
