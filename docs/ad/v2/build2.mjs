 ...
 *
 *   out/founderfloor-ad2-1080p.mp4  (silent, from render.mjs)
...
const OUT = "out";
const MASTER = `${OUT}/founderfloor-ad2-1080p.mp4`;
const SILENT = `${OUT}/founderfloor-ad2-silent.mp4`;
...
const WITH_SOUND = `${OUT}/founderfloor-ad2-sound.mp4`;
ff([
  "-i", SILENT, "-i", BED,
...
console.log("[build2] 16:9 with sound");

const A = ["-c:a", "aac", "-b:a", "160k", "-movflags", "+faststart"];

// ---- 720p travel copy ----
ff(["-i", WITH_SOUND, "-vf", "scale=1280:720:flags=lanczos",
    "-c:v", "libx264", "-crf", "21", "-preset", "slow", ...A, `${OUT}/founderfloor-ad2-720p.mp4`]);
...
ff([
  ...
  "-map", "[v]", "-map", "0:a",
  "-c:v", "libx264", "-crf", "20", "-preset", "slow", ...A, `${OUT}/founderfloor-ad2-square.mp4`,
]);
...
ff([
  ...
  "-map", "[v]", "-map", "0:a",
  "-c:v", "libx264", "-crf", "20", "-preset", "slow", ...A, `${OUT}/founderfloor-ad2-vertical.mp4`,
]);

// ---- webm for the site ----
ff(["-i", SILENT, "-c:v", "libvpx-vp9", "-crf", "33", "-b:v", "0", "-row-mt", "1",
    "-speed", "2", "-an", `${OUT}/founderfloor-ad2.webm`]);

// ---- poster ----
ff(["-i", MASTER, "-ss", "21.20", "-frames:v", "1", `${OUT}/founderfloor-ad2-poster.png`]);

for (const f of ["founderfloor-ad2-sound.mp4", "founderfloor-ad2-silent.mp4", "founderfloor-ad2-720p.mp4",
                 "founderfloor-ad2-square.mp4", "founderfloor-ad2-vertical.mp4",
                 "founderfloor-ad2.webm", "founderfloor-ad2-poster.png"]) {
  const p = `${OUT}/${f}`;
