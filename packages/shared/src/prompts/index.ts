...
Every task on the plan opens its own page (/task) the desk wrote: steps with a tip each, how long, when it is done; every step opens a room where the founder writes what they did and the desk answers; when the last step is ticked the task asks how it went. Every week gets read back (/review): a score out of 100 from what was ticked, written and shown up for, what went well, what to fix, three things to do.
The Workshop (/workshop): the start-up mocked up as a working first version of the product, three tappable screens designed from the sign, the audience and what customers said, with a picture to show people, a build brief, and prompts ready to paste into Lovable, Bolt or v0 (for founders who do not code) or Claude Code (for those who do). This is where mock-ups, prototypes, wireframes, designs and "build it for me" requests go.
