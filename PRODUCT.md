...
- A mock-up's pictures are pictures of something. The building cannot load a photograph — a page that reaches a photo service breaks offline, leaks where the founder is, and carries a stranger's licence — so every picture is drawn inside the document, in the product's own palette, of the thing the product deals in: a loaf, a room, a van, a person. Abstract shapes are the fallback for a noun with no drawing, never the default.
- The studio draws the app on Free and the model draws it on Pro, and both are held to the same standard: one design system declared at the top, no colour outside it, and pictures of real things.
- The finish is technical, not soft. Surfaces carry a hairline and no shadow, corners are 10 to 12, figures are set in a monospaced face with tabular widths so a column of money reads as data, captions are small letterspaced capitals in that same face, a state is an outlined tag rather than a pastel lozenge, and initials sit on a neutral ground in the page's own ink. The drawn path, the model's and the builder hand-off are all held to it.
- What the Workshop draws is a phone app, so it looks like one. The first take is the platform's own chassis with the product's own palette — a near-neutral ground with content in groups on it, the brand on the controls rather than the page, separators that start where the text starts, the system type at the system's sizes, and a translucent bar over a hairline. A category with a deliberate departure (print, terminal, anti-polish) or one that is dark by nature keeps its own look; every other style is a seed away, on "design it again".
- The hand-off to a builder tool carries the design, not only the words. The prompt opens with this app's own tokens already converted to HSL in the shape Lovable, Bolt, Base44 and v0 write files in, then the rule that no colour may appear outside them, and only then the screens — so what the founder gets back is the app they were looking at, not a different one with the same copy.
- The front door leads with a promise. The sign is cut to a headline by removing empty frames ("an app that", "allows you to") and nothing else: no word is added, reordered or respelled, because their words are theirs, including the typos.
