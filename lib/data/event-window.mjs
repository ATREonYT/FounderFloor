...
/**
 * Dated ONE-OFF windows that take precedence over the weekly rule.
 *
 * Authored as Berlin wall clock, like everything in this file, and
 * converted with the same wallToUtc — so a special window inherits the
 * DST-correctness rather than re-solving it. An entry whose end has
 * passed is inert; there is nothing to clean up.
 *
 * `label` replaces the usual "Open Doors · Sunday 18:00 CET" wording
 * everywhere the window is announced (pill, cards, reminder subject).
 *
 * @type {{year: number, month: number, day: number, startHour: number,
 *         endHour: number, label: string}[]}
 */
export const SPECIAL_WINDOWS = [
  // Launch day: doors open ALL day, not just the evening window.
  // DATE: Sunday 30 Aug 2026 — the first Sunday after this shipped; if
  // launch moves, this line is the only thing to move.
  {
    year: 2026,
    month: 8,
    day: 30,
    startHour: 10,
    endHour: 21,
    label: "Launch day — doors open all day",
  },
];

/** The current-or-next SPECIAL window, or null when none remain. */
function nextSpecial(nowMs) {
  let best = null;
  for (const s of SPECIAL_WINDOWS) {
    const startMs = wallToUtc(s.year, s.month, s.day, s.startHour);
    const endMs = wallToUtc(s.year, s.month, s.day, s.endHour);
    if (endMs <= nowMs) continue;
    if (!best || startMs < best.startMs) {
      best = { startMs, endMs, live: nowMs >= startMs, label: s.label };
    }
  }
  return best;
}

/** Every special window that has not finished yet, soonest first — for
 * the calendar file and the admin console. */
export function upcomingSpecialWindows(nowMs) {
  const out = [];
  for (const s of SPECIAL_WINDOWS) {
    const startMs = wallToUtc(s.year, s.month, s.day, s.startHour);
    const endMs = wallToUtc(s.year, s.month, s.day, s.endHour);
    if (endMs > nowMs) out.push({ startMs, endMs, label: s.label });
  }
  return out.sort((a, b) => a.startMs - b.startMs);
}

/**
 * The current-or-next window relative to nowMs — weekly rule plus the
 * SPECIAL_WINDOWS overrides.
 *
 ...
 * started before midnight UTC can still be running. A special window wins
 * whenever it is live or starts no later than the weekly candidate — on
 * launch day (10:00-21:00 over the usual 18:00-21:00) the special simply
 * IS the day's window.
 *
 * `label` is present only when the returned window is a special one.
 *
 * @param {number} nowMs
 * @returns {{startMs: number, endMs: number, live: boolean, label?: string}}
 */
export function nextWindow(nowMs) {
  const special = nextSpecial(nowMs);
  if (special?.live) return special;
  let weekly = null;
  for (let delta = -1; delta <= 8; delta++) {
    ...
    if (endMs > nowMs) {
      weekly = { startMs, endMs, live: nowMs >= startMs };
      break;
    }
  }
  if (!weekly) {
    ...
    weekly = { startMs, endMs: startMs + DURATION_MS, live: false };
  }
  if (special && special.startMs <= weekly.startMs) return special;
  return weekly;
}
